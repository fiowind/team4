...
decl_event!(
	pub enum Event<T>
		...
		ClaimPriceChanged(AccountId, Vec<u8>, Price),
		ClaimPurchased(AccountId, Vec<u8>, Price),
	}
);
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn attach_claim_price(origin, claim: Vec<u8>, price: BalanceOf<T>) -> dispatch::DispatchResult {
			...
			ProofsPrice::<T>::insert(&claim, price.clone());

			Self::deposit_event(RawEvent::ClaimPriceChanged(sender, claim, price));
		...
		/// Purchase claim
		#[weight = 0]
		pub fn purchase_claim(origin, claim: Vec<u8>, bid: BalanceOf<T>) -> dispatch::DispatchResult {
			let sender = ensure_signed(origin)?;
			...
			let price = ProofsPrice::<T>::get(&claim);

			ensure!(bid >= price, Error::<T>::ClaimPriceTooLow);

			T::Currency::transfer(&sender, &owner, price, AllowDeath)?;

			Proofs::<T>::insert(&claim, (&sender, system::Module::<T>::block_number()));

			Self::deposit_event(RawEvent::ClaimPurchased(sender, claim, price));
